...
import {ModelType} from "../../DateCatalog/Catalog";

type PropsType = {
    catalog: ModelType[]
    item: string
}
export const Sneakers = (props: PropsType) => {
    return (
        <div>
            <h2>{props.item}</h2>
            <div className={styles.imgBlock}>
                {props.catalog.map((sneaker, index) => (
                    <Link key={index} to={`${sneaker.id}`}><img src={sneaker.picture} alt={sneaker.model}
                                                               className={styles.images}/></Link>
                ))}
            </div>
            <p>
                What is Lorem Ipsum?
